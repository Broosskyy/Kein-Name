...
export class BossAttackSystem{
 ...
 availableKinds(cycle:number,hpRatio:number):BossAttackKind[]{const phase=hpRatio>.7?1:hpRatio>.4?2:3;const kinds:BossAttackKind[]=phase===1?['ground-slam','falling-debris']:phase===2?['ground-slam','core-beam','void-cone','falling-debris']:['core-beam','void-cone','corruption-ring','shockwave','moving-hazard'];if(cycle>=2&&!kinds.includes('core-beam'))kinds.push('core-beam');if(cycle>=3&&!kinds.includes('corruption-ring'))kinds.push('corruption-ring');return kinds}
 force(kind:BossAttackKind,target:Vec2,cycle=1):BossTelegraph{const d=BOSS_ATTACKS[kind],origin={x:1600,y:120},length=Math.max(1,Math.hypot(target.x-origin.x,target.y-origin.y));const a:BossTelegraph={id:`boss-attack-${++this.sequence}`,kind,phase:'telegraph',position:{...target},origin,direction:{x:(target.x-origin.x)/length,y:(target.y-origin.y)/length},radius:d.radius,innerRadius:d.innerRadius??0,elapsedMs:0,damage:Math.round(d.baseDamage*(1+(cycle-1)*.22)),hitApplied:false};this.active.push(a);return a}
 update(deltaMs:number,player:Vec2,cycle:number,enabled=true,hpRatio=1):BossAttackImpact[]{const impacts:BossAttackImpact[]=[];if(!enabled)return impacts;this.cooldownMs-=deltaMs;if(this.cooldownMs<=0&&this.active.length<3){let pool=this.availableKinds(cycle,hpRatio).filter(k=>!this.recent.slice(-2).includes(k));if(!pool.length)pool=this.availableKinds(cycle,hpRatio);const kind=pool[Math.floor(this.random.next()*pool.length)],jitter=()=>(this.random.next()-.5)*620,target={x:clamp(player.x+jitter(),260,2940),y:clamp(player.y+jitter()*.6,260,1540)};this.force(kind,target,cycle);this.recent.push(kind);if(this.recent.length>4)this.recent.shift();this.cooldownMs=Math.max(1050,(3100-cycle*220)*(.88+this.random.next()*.25))}
