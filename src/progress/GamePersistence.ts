...
export class GamePersistence {
  ...
  loadRun(): ArenaRunSnapshot | undefined {
    try { const raw = this.storage.getItem(RUN_KEY); if (!raw) return undefined; const parsed = JSON.parse(raw) as ArenaRunSnapshot; return parsed.schemaVersion === 1 && parsed.eligible ? parsed : undefined; }
